<template>
  <v-dialog id="budget-dialog" max-width="800" v-model="dialogIsShow">
    <v-card>
      <v-card-title class="headline primary-gradient white--text py-2">
        <span>{{ $t('budget_popup', 'common') }}</span> 
        <v-spacer></v-spacer>
                ...
                <v-card outlined tile>
                  <v-container fluid>
                    <v-row dense align="center" justify="space-between">
                      <v-col>
                        <v-text-field clearable dense hide-details outlined append-icon="mdi-magnify" :label="$t('budget_name')" @keypress.enter="search" v-model="searchText" @click:append="search"></v-text-field>
                      </v-col>
                    </v-row>
                  </v-container>
                </v-card>
                ...
                <v-card outlined tile v-resize="onResize">
                  <DxDataGrid column-resizing-mode="widget" key-expr="PK" 
                    ...
                    :onRowDblClick="onSelectSingle">
                      <DxColumn data-field="CODE" :caption="$t('budget_name')"></DxColumn>
                      <DxColumn data-field="NAME" :caption="$t('budget_lname')"></DxColumn>
