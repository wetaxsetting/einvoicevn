...
<template>
<div>
  <v-dialog id="free-item-service-dialog" max-width="1200" v-model="dialogIsShow">
      <v-card>
        <v-card-title class="headline primary-gradient white--text py-2">
          <span>{{ $t("free_item_service") }}</span>
          <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    async LG_SEL_FREEITEM_DIALOG_1(_param) {
      const dso = {
        type: 'grid',
        selpro: 'LG_SEL_FREEITEM_SERVICE_DIALOG_1',
        para: _param
      }
    ...
    async LG_LST_FREEITEM_DIALOG(_param) {
			const dso = {
        type: 'list',
        selpro: 'LG_LST_FREEITEM_SERVICE_DIALOG',
        para: _param
			}
			switch(_param[0]){
        case 'ITEMGRP':
