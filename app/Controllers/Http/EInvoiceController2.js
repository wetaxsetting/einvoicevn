    ...
    try {
      ...
      if (json_xml[0].xml_process) {
        ...
        if (process_type == 'I' || process_type == 'P') {
          ...
          // --- Ký xong rồi: xử lý từng hóa đơn theo chỉ số i ---
          const data_inv = [];
          const data_error = [];

          for (let i = 0; i < json_xml_signed.data.length; i++) {
            // 1) Extract theo TỪNG hóa đơn (không “mượn” phần tử [0])
            ...
            const masterInvoicePK = await this.weTaxExtractNorXMLContent(
              ...
              buyerEmail,
              buyerEmailCC,
              '', '',                // các trường trống như hiện trạng
              tax_serial_number,
              reqKey,
              '',                    // mccqt (nếu cần để riêng, có thể truyền list_invoice[i].mccqt)
              p_language,
              p_crt_by,
            );

            // 2) Kiểm tra trạng thái đăng ký phát hành / ngày hóa đơn
            if (masterInvoicePK.PK == -1) {
              data_error.push({ req_key: reqKey, errmsg: 'The issuer invoice has not register' });
              continue;
            }
            if (masterInvoicePK.PK == -2) {
              data_error.push({ req_key: reqKey, errmsg: 'invoice date current < invoice date max' });
              continue;
            }
            if (masterInvoicePK.PK < -2) {
              data_error.push({ req_key: reqKey, errmsg: 'Invalid xml format' });
              continue;
            }

            // 3) Nếu đã từng gửi & đã có MCCQT: trả lỗi “đã gửi” cho hóa đơn này rồi NEXT các hóa đơn khác
            if (masterInvoicePK.PK == 0 && masterInvoicePK.CQT_MCCQT) {
              data_error.push({ req_key: reqKey, errmsg: 'The sign xml was send Tax Office' });
              continue;
            }

            // 4) Chuẩn bị mapping CHÍNH XÁC cho hóa đơn i (KHÔNG dùng phần tử [0])
            const oneTrade = [{
              sale_id:       list_invoice[i].sale_id,
              req_ep_key:    masterInvoicePK.REQ_KEY_PK,
              trade_code:    masterInvoicePK.MA_TRACUU,    // sẽ bị overwrite sau khi POST
            ...
            // 5) Gửi hóa đơn thường i (tuần tự để tránh rate limit phía TVAN/CQT)
            ...
            // 6) Gom kết quả nếu có
            if (data_send_tax?.rtnValue?.[0]) {
              const r = data_send_tax.rtnValue[0];
              data_inv.push({
                trade_code:         data_send_tax.trade_code,
                mccqt:              r.mccqt,
                tax_code:           list_invoice[i].seller_taxcode,
                form_no:            list_invoice[i].form_no,
                serial_no:          list_invoice[i].serial_no,
                invoice_no:         list_invoice[i].invoice_no,
                inform_code:        r.inform_code,
                inform_name:        r.inform_name,
                lookup_code:        r.lookup_code,
                sign_datetime:      r.sign_datetime,
                sign_by:            r.sign_by,
                xml_no_sign:        json_xml[0].xml,                 // giữ nguyên như trước
                xml_signed:         json_xml_signed.data[i].signed_xml, // SỬA về i
                xml_tax_signed:     r.xml_tax_signed,
                tax_sign_by:        r.tax_sign_by,
                tax_sign_datetime:  r.tax_sign_datetime,
              });
            } else {
              data_error.push({ req_key: reqKey, errmsg: 'Send/Check to TAX failed or empty rtnValue' });
            }
          }

          const res_data = {
            seller_tax_code:  list_invoice[0]?.seller_taxcode || tax_code,
            sale_date:        list_invoice[0]?.sale_date      || sale_date,
            store_code:       list_invoice[0]?.store_code     || store_code,
            store_name:       list_invoice[0]?.store_name     || store_name,
            tax_serial_number:list_invoice[0]?.tax_serial_number || tax_serial_number,
            data_error,
            data_inv,
          };

          const ok = data_inv.length > 0 && data_error.length === 0;
          const message = ok ? 'Sending invoice is successfully.'
                            : (data_inv.length > 0 ? 'Partially sent. Some invoices failed.' : 'All invoices failed.');
          const code = ok ? 200 : 409;
          return response.status(code).json(Utils.responseByRule({ success: ok, message, data: res_data }));
        }
      } else {
    ...
    try {
      ...
      if (process_type == 'E') {
        ...
        for (let i = 0; i < invoices.length; i++) {
          ...
          if (isNaN(last_invoice_no)) {
            json_xml.push({
            error: `Failed to create invoice no. Please contact administrator for helping.`,
            xml_process: false,
          });
          return {json_xml, data_send_mail};
          ...
          if (Number(invoices[i].invoice_date.replaceAll("-","")) < Number(last_invoice_date.replaceAll("-",""))) {
             json_xml.push({
              error: `invoice date cannot smaller than ${last_invoice_date}.`,
              xml_process: false,
            });
            return {json_xml, data_send_mail};
          
          }
          if (Number(invoices[i].invoice_date.replaceAll("-","") >= Number(tomorrow_date.replaceAll("-","")))) {
            json_xml.push({
              error: `invoice date cannot greater than ${tomorrow_date}.`,  
             xml_process: false,
            });
            return {json_xml, data_send_mail};
          }
          ...
          if(invoices[i].version != "2.1.0")
          {
            ...
            objInvoice.HDon.DLHDon.NDHDon.NMua.HVTNMHang = this.convertHtmlCode(invoices[i].buyer_nm);
          ...
          objInvoice.HDon.DLHDon.NDHDon.NBan.MCHang = store_code;
          objInvoice.HDon.DLHDon.NDHDon.NBan.TCHang = store_name;
          objInvoice.HDon.DLHDon.NDHDon.NBan.STKNHang = invoices[i].seller_bank_no || ' ';
          objInvoice.HDon.DLHDon.NDHDon.NBan.TNHang = invoices[i].seller_bank_name || ' ';
          ...
          objInvoice.HDon.DLHDon.NDHDon.NMua.MDVQHNSach = "";
        ...
        for (let i = 0; i < invoices.length; i++) {
          ...
          }else
          {
            ...
            objInvoice.HDon.DLHDon.NDHDon.NBan.Fax = invoices[i].seller_fax || '';
            objInvoice.HDon.DLHDon.NDHDon.NBan.Website = invoices[i].seller_website || '';
            objInvoice.HDon.DLHDon.NDHDon.NBan.MCHang = invoices[i].store_code || '';
            objInvoice.HDon.DLHDon.NDHDon.NBan.TCHang = invoices[i].store_name || '';
            objInvoice.HDon.DLHDon.NDHDon.NBan.TTKhac = '';
            ...
            objInvoice.HDon.DLHDon.NDHDon.NMua.MDVQHNSach = this.convertHtmlCode(invoices[i].budget_relationship_unit_code) || '';
            objInvoice.HDon.DLHDon.NDHDon.NMua.CCCDan = this.convertHtmlCode(invoices[i].ccid) || '';
            objInvoice.HDon.DLHDon.NDHDon.NMua.SHChieu = this.convertHtmlCode(invoices[i].passport) || '';
            ...
            objInvoice.HDon.DLHDon.NDHDon.TToan.TgTCThue = invoices[i].total_amt || '0';
            objInvoice.HDon.DLHDon.NDHDon.TToan.TgTThue = invoices[i].total_vat_amt || '0';
            objInvoice.HDon.DLHDon.NDHDon.TToan.TTCKTMai = invoices[i].total_dc_amt || '0';
            objInvoice.HDon.DLHDon.NDHDon.TToan.TgTTTBSo = invoices[i].total_payment || '0';
            objInvoice.HDon.DLHDon.NDHDon.TToan.TgTTTBChu = invoices[i].total_payment_word_vie; //await Utils.Num2VNText2(invoices[i].total_payment.toString() || '0', invoices[i].currency);
  ...
  weTaxValidateNormalInvoiceToXML(invoices) {
    ...
    try {
      const errorList = {
        ...
        total_vat_list: {
          sub_vat_rate:  /^(0|5|8|10)%$|^KCT$|^KKKNT$|^CTTC$|^KHAC(:\d+(\.\d+)?%)?$/, ///^(0%|5%|8%|10%|KCT|KKKNT)$/, //sub_vat_rate: /^(0%|5%|8%|10%)$/, //  /^-?\d+(\.\d{1,4})?[%]/, //11,
          sub_amt: /^-?[0-9]{0,21}(?:\.[0-9]{1,6})?$/,
          sub_amt_vat: /^-?[0-9]{0,21}(?:\.[0-9]{1,6})?$/,
        },
