  ...
  <div>
    <v-card flat tile :color="!breakpoint.isDesktop ? 'primary-radial-gradient' : 'transparent'" :height="windowHeight">
      <v-img
        ...
        v-if="breakpoint.isDesktop"></v-img>
        ...
        <v-card-text class="pb-0">
          <v-form lazy-validation ref="form">
            <v-text-field
              ...
              outlined
              validate-on-blur
              ref="converterInputRefs"
              label="Họ tên người chuyển đổi"
              :color="currentTheme"
              :disabled="isProcessing"
              :rules="inputRule"
              v-model="inputName"></v-text-field>
          </v-form>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn depressed :color="currentTheme" :class="currentTextColor" :disabled="isProcessing" :loading="isProcessing" @click="convert"
            >Tiếp tục</v-btn
          >
  ...
  methods: {
    ...
    async search() {
      ...
      try {
        ...
        const {success, data, message} = await this.$axios.$post('/einvoice/lookup-minutes', {
