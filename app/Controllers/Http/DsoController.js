...
class DsoController {
    ...
    async htmlToPdf({ request, response }) {
        try {
            ...
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir, { recursive: true }, err => { console.err(err) })
            }
    ...
    async writeRTDB({ request, response, auth }) {
        try {
            ...
            if (result != "OK") {
                return response.send(
                    Utils.response(false, result, para)
                )
            } else {
                return response.send(
    ...
    async getSessions({ response, auth }) {
        try {
            ...
            const user = await auth.getUser();
            return response.send(
                Utils.response(true, "Successfull call getSessions", _user_last_called)
            );
        } catch (e) {
            Utils.Logger({
                LVL: "error",
    ...
    async CallProcedure({ request, response, auth }) {
        try {
            ...
            if (proc.toUpperCase().indexOf("_UPD_") > 0 || proc.toUpperCase().indexOf("_PROCESS_") > 0 ||
                proc.toUpperCase().indexOf("_PRO_") > 0 || proc.toUpperCase().indexOf("_DEL_") > 0 ||
                proc.toUpperCase().indexOf("_COPY_") > 0 || proc.toUpperCase().indexOf("_REGEN_") > 0 || proc.toUpperCase().indexOf("_NOCACHE") > 0) {
                            ...
                            if (idleTime >= API_SESSION_TIMEOUT) {
                                ...
                                if (proc.toUpperCase() != "SYS_SEL_LANGUAGE") {
                                    return response.send(
                                        Utils.response(
                                            false,
                                            "your_session_timeout_logout_and_login_to_continue", { lang: p_language }
                                        )
                                    );
                                } else {
                                    _user_last_called[i].lastcalled = miliseconds;
                ...
                for (let i = 0; i < result.length; i++) {
                    if (result[i].DEVICE_ID && result[i].TITLE) {
                        ...
                        await DBService.callProcCursor(
                            "LG_WMS_UPD_NOTICE_LOG", [result[i].PK],
                            p_language,
                            p_crt_by
                        );
                    }
    ...
    async DownloadFile({ request, response, auth }) {
        try {
            ...
            result = await DBService.ExecuteSQL(
                `select * from ${table_name} where pk=:pk`, { pk: pk },
                p_crt_by,
                _db2
            );
            if (DB_CONNECTION == "oracle") {
    ...
    async createDictionaryAuto({ request, response, auth }) {
        ...
        2
            ...
            if (_message_ids.length > 0) {
                await DBService.callProcCursor(
                    "sys_upd_dictionary_auto", [_message_ids.join(), menu_cd ? menu_cd : fileid],
                    p_language,
                    p_crt_by,
                    _db2
                );
