<template>
  <v-dialog id="view-einvoicedialog" width="1000" v-model="dialogIsShow" overlay-opacity="0.1" :origin="origin" :transition="transition" :fullscreen="isMaximized">
    ...
    <input type="textbox" id="04SS-tempPK" v-show="false" />
...
export default {
  ...
  methods: {
    previewCellFileD() {
      this.currentRow = document.getElementById("04SS-tempPK").value;
    ...
    myCellHTML(row, column, value, cellhtml) {
      ...
      let html = `<button class="v-icon mdi mdi-eye light-blue--text px-16" onclick="document.getElementById('04SS-tempPK').value = '${rowData.PK}';${previewXML}"></button>`;
      return html;
    },
