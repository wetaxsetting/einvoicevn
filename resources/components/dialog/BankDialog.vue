                            ...
                            <v-card outlined tile>
                                <v-container fluid>
                                    <v-row dense align="center" justify="space-between">
                                        ...
                                        <v-col cols="4">
                                            <v-text-field clearable dense hide-details outlined :label="$t('bank_code', 'common')" @keypress.enter="onSearch" v-model="txtBankId"></v-text-field>
                                        </v-col>
                                        <v-col>
                                            <v-text-field clearable dense hide-details outlined :label="$t('bank_name', 'common')" @keypress.enter="onSearch" v-model="txtBankName"></v-text-field>
                                        </v-col>
...
export default {
    ...
    computed: {
        defaultHeaders() { 
            ...
            //  console.log('vng-154-dvg^_^: > file: BankDialog.vue > line 98 > defaultHeaders > defaultHeaders', this.headers);
            return [{
                    field: "BANK_ID",
                ...
                {
                    field: "ACCOUNT_NO",
                    caption: this.$t("account_no", "acnt"),
                    width: "100"
                } 
            ];
        },
    ...
    watch: {
        ...
        dialogIsShow(val) {
            if (val === true) {
                ...
                this.dataList = [];
                this.selectedDatas = [];
                //  console.log('vng-154-dvg^_^: > file: BankDialog.vue > line 137 > dialogIsShow > company', this.company);
                if (this.company != 0) {
    ...
    methods: {
        ...
        async onSearch() {
            const dso = {
                type: "grid",
                selpro: "sys_sel_bank_dialog",
                para: [
                    this.selectedCompany,
                    this.txtBankId ? this.txtBankId : "",
                    this.txtBankName ? this.txtBankName : ""
                ]
            };
            this.dataList = await this._dsoCall(dso, "select", false);
            this.$refs.grid_bank.instance.clearSelection();
        },
