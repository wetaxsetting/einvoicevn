                    ...
                    <v-container fluid class="pb-0">
                      <v-row dense align="start" justify="space-between">
                        ...
                        <v-col md="3" sm="6" cols="12" class="pb-sm-0 pb-2">
                          <v-select cache-items clearable dense hide-details item-value="CODE" item-text="NAME" :label="$t('language', 'common')" :items="item_languages" v-model="selectedLang"></v-select>
                        </v-col>
                ...
                <v-col cols="12" class="py-0">
                  <v-card outlined tile>
                    <DxDataGrid column-resizing-mode="widget" key-expr="PK" ref="dictionaryGrid"
                      ...
                      :paging="{ pageSize: dictionaryList.length }" :selection="{ mode: 'multiple', showCheckBoxesMode: 'none' }" 
    ...
    <v-dialog persistent id="copy-to-dialog" max-width="350" v-model="copyToDialog">
      <v-card>
        ...
        <v-card-text class="pa-4 pb-2 d-flex flex-column align-space-between">
          <v-select cache-items clearable dense hide-details outlined class="pb-3" item-value="CODE" item-text="NAME" :error="error1" :label="$t('from_language')" :items="item_languages" v-model="selectedLang"></v-select>
...
export default { 
  ...
  async created() {
    //await this.dictionaryAuto()
     ...
     const commoncode = await this._getCommonCode( "COAB0070",  0 );
    var result = [];
    var map = new Map();
    for(const item of commoncode)
    {
    ...
    console.log(result);
    this.item_languages = result; 
    this.selectedLang = this.item_languages[0].CODE
    this.dictionaryFormID = this.formID
    this.toLangList = result
    this.dictionaryTableHeaders1(result);
  },
  ...
  methods: {
            ...
            dictionaryTableHeaders1(p_list) {
             // console.log(p_list)
              return this.dictionaryTableHeaders = [
    ...
    async getDictionaryList(paramsData) {
      const dso = { type: "grid", selpro: "SYS_SEL_SYSE003_DICTIONARY_RPT", para: paramsData };
    ...
    save() {
      this.$refs.dictionaryGrid.instance.saveEditData().then(async () => {
        ...
        const dso = {
          type: "grid",
          selpro: "SYS_SEL_SYSE003_DICTIONARY_RPT",
          updpro: "SYS_UPD_SYSE003_DICTIONARY_RPT",
          para: [
            this.keyword ? this.keyword : "",
            this.message ? this.message : "",
            this.selectedLang ? this.selectedLang : "",
    ...
    copy() {
      ...
      this.$axios.$post("dso/callproc", { proc: "SYS_SEL_SYSE003_COPY_RPT", para: data })
        .then(res => {
          if (res.data) {
