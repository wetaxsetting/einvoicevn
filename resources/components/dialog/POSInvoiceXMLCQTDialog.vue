<template>
  <v-dialog id="view-einvoicedialog" width="1000" v-model="dialogIsShow" overlay-opacity="0.1" :origin="origin" :transition="transition" :fullscreen="isMaximized">
    ...
    ></view-einvoice-xml-dialog>
    <button type="button" v-show="false" :id="`POSInvoice-btnPrviewHD`" @click="previewCellFile"></button>
    <input type="textbox" id="POSInvoice-tempPK_HD" v-show="false" />
...
export default {
  ...
  methods: {
    previewCellFile() {
      this.currentRow = document.getElementById("POSInvoice-tempPK_HD").value;
      const ds = this.$refs.grdKQCQT.getDataSource();
        this.xmlUrl = ds[0].CQT_DATA_RESULT;
        console.log("this.xmlUrl  ", this.xmlUrl);
    ...
    myCellHTML(row, column, value, cellhtml) {
      ...
      let previewXML = `document.getElementById('POSInvoice-btnPrviewHD').click()`;
      let html = `<button class="v-icon mdi mdi-eye light-blue--text px-16" onclick="document.getElementById('POSInvoice-tempPK_HD').value = '${rowData.PK}';${previewXML}"></button>`;
      return html;
    },
