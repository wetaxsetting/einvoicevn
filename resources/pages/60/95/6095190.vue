      ...
      <v-col cols="12">
        <v-card class="pa-2" :color="$vuetify.theme.dark ? 'grey darken-3' : 'grey lighten-4'">
          <v-row class="py-0">
            ...
            <v-col md="2">
              <BaseSelect outlined :label="$t('form_no')" item-text="NAME" item-value="VAL" :lstData="from_no_list" v-model="selected_form_no" />
            </v-col>
            <v-col md="1">
              <BaseSelect
                outlined
            ...
            <v-col md="1">
              <BaseSelect outlined :label="$t('status')" item-value="VAL" item-text="NAME" :lstData="status_list" v-model="selected_status" />
            </v-col>
            <v-col md="1">
              <BaseSelect
                outlined
...
export default {
  ...
  methods: {
    ...
    async OnSuccessCallReUploadPDFS() {
      let pdfFile = await this._callProcedure('EI_SEL_GET_PDF_FILE_DYNAMIC', [this.tei_einvoice_m_pk_row]);
      pdfFile.forEach(e => {
    ...
    async getListCodes(pos) {
      switch (pos) {
        ...
        case 'form_no':
          ...
          this.from_no_list = await this._dsoCall(dso_form_no_list, 'select', false);
        ...
        case 'serial_no':
          ...
          this.serial_no_list = await this._dsoCall(dso_serial_no_list, 'select', false);
        ...
        case 'status':
          ...
          this.status_list = await this._dsoCall(dso_status_list, 'select', false);
        ...
        case 'trading_type':
          ...
          this.trading_type_list = await this._dsoCall(dso_trading_type_list, 'select', false);
