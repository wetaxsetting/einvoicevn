      ...
      <v-col md="12">
        <v-row dense class="pa-3">
          <v-col md="3" class="pl-2 pr-2">
            <BaseSelect outlined :label="$t('company')" v-model="selected_company" :lstData="company_list" item-text="NAME" item-value="VAL" />
          </v-col>
          ...
          <v-col md="2" class="pl-2 pr-2">
            <BaseSelect outlined :label="$t('converted_yn')" :lstData="yn_list" v-model="selected_yn" item-text="NAME" item-value="VAL" filter_off />
          </v-col>
          <v-col md="1" class="pl-2 pr-2">
    ...
    <v-row dense>
      <v-col md="12">
        ...
        <BaseGridView
          ref="grdConvert"
          :header="grdHeader"
          sel_procedure="EI_SEL_6095120_DATA_NC"
          :multiselect="true"
...
export default {
  ...
  methods: {
    ...
    async getListCodes() {
      let company = await this._callProcedure('EI_SEL_6095120_COMPANY', [this.user.PK]);
      this.company_list = company;
      this.selected_company = this.company_list[0].VAL;
      ///////////////////////////////////////////////////////////
      let tradingType = await this._callProcedure('EI_SEL_6095120_TRADING_TYPE');
      this.trading_type_list = tradingType;
      this.selected_trading_type = this.trading_type_list[0].VAL;
      /////////////////////////////////////////////////////////////////
      let convertYN = await this._callProcedure('EI_SEL_6095120_SEL_CONVERTYN');
