<template>
  <v-container>
     <v-dialog id="mold-dialog" max-width="1000" v-model="dialogIsShow">
      <v-card>
        <v-card-title class="headline primary-gradient white--text py-2">{{ $t('pop04_emp_mapping') }}</v-card-title>
                      ...
                      <v-row align="center" justify="space-between" class="pa-0">
                        <v-col cols="12" class="pt-0">
                          <DxDataGrid
                            ...
                            :height="limitHeight"
                            :no-data-text="$t('pop04_no_data', 'common')"
                            :show-borders="true"
                          ...
                          >
                             <DxColumn
                              data-field="PROD_DATE"
                              :caption="$t('pop04_pro_date')"
                              data-type="date"
                              :format="curLang.DATE_FORMAT"
                              :allow-editing="false"
                              width="80"
                            ...
                            <DxColumn
                              data-field="SEQ"
                              :caption="$t('pop04_seq', '4070050_pop04')"
                              data-type="string"
                              :allow-editing="false"
                              width="60"
                              css-class="cell-align-left"
                            />
                            <DxColumn data-field="CHARGER_PK" :allow-editing="false" :visible="false" />
                            <DxColumn 
                              data-field="EMP_ID" 
                              :caption="$t('pop04_none_type', '4070050_pop04')"
                              width="100"
                              :allow-editing="false"                              
                              css-class="cell-align-left">                             
                            </DxColumn>                           
...
export default {
  ...
  mounted() {
    /*TODO: */  
      //this.LG_LST_4070050_pop04(["NONEWORKTYPE", "", this.user.PK]);   
     //this.DSO_LG_4070050_pop04_1("select");
    // this.LG_LST_4070050_pop04(["CONFIRM_YN", this.model_Right.TLG_PB_FACTORY_PK, this.user.PK]);
  },
  computed: {
    user() {
  ...
  methods: { 
    ...
    /*NOTE: */
    async DSO_LG_4070050_pop04_1(action = "update") {
      //const dataIsModified = this.dataGrid1.some(x => x._rowstatus !== "")     
      //this.$refs.grdDetail.instance.saveEditData();
      const dso = {
        type: "grid",
        selpro: "LG_SEL_4070050_pop04_1",
        updpro: "LG_UPD_4070050_pop04_1",
        para: [
          this.dialogDate,
          this.dialogShift,
          this.dialogLinePK,
          this.dialogWProcessPK,
          this.dialogItemPK,
    ...
    /*NOTE: */
    async LG_LST_4070050_pop04(_param) {
      const dso = {
        type: "list",
        selpro: "LG_LST_4070050_pop04",
        para: _param
      };
      switch (_param[0]) {       
        case "NONEWORKTYPE":
