<template>
    ...
    >
          ...
          <v-row no-gutters align="center" justify="center">
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewSetting_' + idKey" depressed icon v-on="on" v-show="gridSetting" class="mx-1 blue--text" style="height: 15px !important; width: 15px !important" @click="myGridSettingClick">
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewExport_' + idKey" depressed icon v-on="on" class="mx-1 green--text" style="height: 15px !important; width: 15px !important" @click="myGridOnExport">
            ...
            <v-tooltip top v-if="exportexcel2">
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewExport2_' + idKey" depressed icon v-on="on" class="mx-1 green--text" style="height: 15px !important; width: 15px !important" @click="exportexcel2">
                  <v-icon small>mdi-microsoft-excel</v-icon>
                </v-btn>
              </template>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewFilter_' + idKey" depressed icon v-on="on" class="mx-1" :class="gridFilterList.filter((q) => q['add'] == true).length > 0 ? 'red--text' : 'black--text'" style="height: 15px !important; width: 15px !important" @click="myGridOnOpenFilter">
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewSet_' + idKey" depressed icon v-on="on" class="mx-1 blue white--text" style="height: 17px !important; width: 17px !important" @click="gridShowSetData = !gridShowSetData">
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewDelete_' + idKey" depressed icon v-on="on" class="mx-1 red white--text" style="height: 17px !important; width: 17px !important" @click="myGridOnDeleteInsert">
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn :id="'gridviewExport_' + idKey" depressed icon v-on="on" class="mx-1 green--text" style="height: 15px !important; width: 15px !important" @click="selectallrows" v-show="autoselectallrows">
                ...
                <td style="width: 30px !important">
                  <v-row no-gutters>
                    ...
                    <v-col cols="12" lg="12" v-show="!isProcessReport">
                      <v-tooltip top>
                        <template v-slot:activator="{ on }">
                          <v-btn :id="'gridviewExport_' + idKey" depressed icon color="#3766c4" v-on="on" style="height: 15px !important; width: 15px !important" @click="onPrintReport" v-bind:disabled.sync="isProcessReport">
      ...
      <v-row no-gutters v-show="gridShowSetData" style="background-color: #e8e9eb !important">
        <v-col cols="12" md="3" v-show="gridSetColText && gridSetColText.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('text_column')" :lstData="gridSetColText" v-model="gridSetColTextSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
        ...
        <v-col cols="12" md="3" v-show="gridSetColList && gridSetColList.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('list_column')" :lstData="gridSetColList" v-model="gridSetColListSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('value')" :lstData="gridSetColListVals" v-model="gridSetColListVal" null class="justify-space-around px-1" style="min-width: 50% !important"> </BaseSelect>
        ...
        <v-col cols="12" md="3" v-show="gridSetColFlag && gridSetColFlag.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('flag_column')" :lstData="gridSetColFlag" v-model="gridSetColFlagSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('value')" :lstData="gridSetColYN" v-model="gridSetColFlagVal" null class="justify-space-around px-1" style="min-width: 50% !important"> </BaseSelect>
        ...
        <v-col cols="12" md="3" v-show="gridSetColDate && gridSetColDate.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('date_column')" :lstData="gridSetColDate" v-model="gridSetColDateSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
        ...
        <v-col cols="12" md="3" v-show="gridSetColMonth && gridSetColMonth.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('month_column')" :lstData="gridSetColMonth" v-model="gridSetColMonthSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
        ...
        <v-col cols="12" md="3" v-show="gridSetColTime && gridSetColTime.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('time_column')" :lstData="gridSetColTime" v-model="gridSetColTimeSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
        ...
        <v-col cols="12" md="3" v-show="gridSetColText && gridSetColText.length > 0">
          <v-card class="d-flex px-1 py-1 mx-1 my-1" outlined>
            <BaseSelect item-value="CODE" item-text="NAME" :label="$t('dialog_column')" :lstData="gridSetColDialog" v-model="gridSetColDialogSelected" null style="width: 25% !important" class="justify-start"> </BaseSelect>
      ...
      <!-- <div :style="{
          ...
          > -->
      <span @contextmenu="contextmenuMenu($event)" @mousedown="mousedown($event)">
        <jqxgrid
          ref="myGrid"
          :name="'myGrid_' + idKey"
      ...
      <!-- </div> -->
      <CustomGridDialog ref="customGridDialog" @callback="customGridCallback" :menu_cd="menu_cd" :grid_id="id" :customTables="customTables" :currentCols="currentCols"></CustomGridDialog>
            ...
            <v-row no-gutters v-for="(item, idx) in gridFilterList" :key="idx" align="center" justify="start">
              <v-col cols="12" md="1" class="px-1 py-1">
                <BaseButton
                  ...
                  :btn_text="$t('add')"
                  @onclick="
                    gridFilterList[idx]['add'] = true;
                    addMoreFilter(idx);
                  "
                  v-if="gridFilterList[idx]['add'] == false"
                />
              ...
              <v-col cols="12" md="3" class="px-1 py-1" v-show="gridFilterList[idx]['add'] == true">
                <BaseSelect :outlined="true" item-value="CODE" item-text="NAME" :label="$t('column')" :lstData="columnNameList" :value="gridFilterList[idx]['column']" @input="gridFilterList[idx]['column'] = $event" @change="changeFilterType(idx)"> </BaseSelect>
              </v-col>
              <v-col cols="12" md="2" v-show="gridFilterList[idx]['add'] == true">
                <BaseSelect :outlined="true" item-value="CODE" item-text="NAME" :label="$t('condition')" :lstData="getFilterCondition(gridFilterList[idx]['type'])" :value="gridFilterList[idx]['condition']" @input="gridFilterList[idx]['condition'] = $event" class="px-1 py-1"> </BaseSelect>
              </v-col>
              <v-col cols="12" md="6" v-show="gridFilterList[idx]['add'] == true">
                ...
                <BaseSelect
                  ...
                  :value="gridFilterList[idx]['value']"
                  @input="gridFilterList[idx]['value'] = $event"
                  class="px-1 py-1"
                >
                </BaseSelect>
  
                <!-- checkbox -->
                <BaseSelect :outlined="true" v-if="gridFilterList[idx]['type'] == 'checkbox'" item-value="CODE" item-text="NAME" :label="$t('value')" :lstData="gridFilterDropDownList['CHECK_BOX']['dataSource']" :value="gridFilterList[idx]['value']" @input="gridFilterList[idx]['value'] = $event" class="px-1 py-1"> </BaseSelect>
                <!-- string -->
                ...
                <v-row no-gutters v-if="gridFilterList[idx]['type'] == 'string'" class="px-0 py-0">
                  <v-col cols="12" :md="gridFilterList[idx]['condition'] == 'BETWEEN' ? 6 : 12" class="px-1 py-1">
                    <BaseInput :outlined="true" :label="$t('value')" v-if="gridFilterList[idx]['type'] == 'string'" :value="gridFilterList[idx]['value']" @input="gridFilterList[idx]['value'] = $event"> </BaseInput>
                  </v-col>
  
                  <v-col cols="12" md="6" class="px-1 py-1" v-show="gridFilterList[idx]['condition'] == 'BETWEEN'">
                    <BaseInput :outlined="true" :label="$t('to_value')" v-if="gridFilterList[idx]['type'] == 'string'" :value="gridFilterList[idx]['tovalue']" @input="gridFilterList[idx]['tovalue'] = $event"> </BaseInput>
                  </v-col>
                </v-row>
                ...
                <v-row no-gutters v-if="gridFilterList[idx]['type'] == 'datetimeinput'" class="px-0 py-0">
                  <v-col cols="12" md="6" class="px-1 py-1">
                    <BaseDatePicker :outlined="true" :label="$t('from_dt')" null v-if="gridFilterList[idx]['type'] == 'datetimeinput'" :value="gridFilterList[idx]['value']" @input="gridFilterList[idx]['value'] = $event"></BaseDatePicker>
                  </v-col>
                  <v-col cols="12" md="6" class="px-1 py-1">
                    <BaseDatePicker :outlined="true" :label="$t('to_dt')" null v-if="gridFilterList[idx]['type'] == 'datetimeinput'" :value="gridFilterList[idx]['tovalue']" @input="gridFilterList[idx]['tovalue'] = $event"></BaseDatePicker>
                  </v-col>
                </v-row>
                <!-- number-->
                <v-row no-gutters v-if="gridFilterList[idx]['type'] == 'numeric'" class="px-0 py-0">
                  <v-col cols="12" md="6" class="px-1 py-1">
                    <BaseInput :outlined="true" :label="$t('value')" v-if="gridFilterList[idx]['type'] == 'numeric'" :value="gridFilterList[idx]['value']" @input="gridFilterList[idx]['value'] = $event" number="2"> </BaseInput>
                  </v-col>
                  <v-col cols="12" md="6" class="px-1 py-1">
                    <BaseInput :outlined="true" :label="$t('to_value')" v-if="gridFilterList[idx]['type'] == 'numeric'" :value="gridFilterList[idx]['tovalue']" @input="gridFilterList[idx]['tovalue'] = $event" number="2"> </BaseInput>
                  </v-col>
                </v-row>
              </v-col>
            </v-row>
  ...
  export default {
    ...
    props: {
      ...
      onRowPrepared: { type: Function, default: null },
      ...
      pagenumber: { type: Number, default: 3 },
    },
    data() {
      return {
        sheets: [
    ...
    computed: {
      limitHeight() {
        ...
        if (!!this.height) {
          _height = this.height;
          try {
            this.$refs.myGrid.height = _height - 28;
          } catch {}
          return _height;
        }
      ...
      cookieId: function () {
        let _cookieId = null;
        try {
          let formId = document.getElementById("appBar").getElementsByClassName("v-chip--active")[0].getElementsByClassName("v-chip__content")[0].innerText.replace(/\s+/g, "_");
    ...
    methods: {
      ...
      cellTooltipValue(cell) {
        ...
        try {
          ...
          if (col.hasOwnProperty("lookup")) {
            ...
            try {
              _cellValue = _dataSource.find((q) => q[_key] == _cellValue)[_name];
            } catch (e) {}
          }
        } catch {}
      ...
      async _onSave() {
        if (this.sel_procedure && this.upd_procedure && this.filter_paras && this.update_paras) {
          ...
          this._console("sel_procedure || upd_procedure || filter_paras || update_paras is undefined!");
      ...
      myGridOnCellClick(e) {
        ...
        if (this.lastCellClick && this.lastCellClick.args.columnindex == e.args.columnindex && this.lastCellClick.args.rowindex == e.args.rowindex && nowClickTime - this.lastCellTimeCllick < this.constTimeDblClick) {
      ...
      myGridOnOpenFilter() {
        if (!this.firstFilter && this.columns) {
          ...
          this.gridFilterCondition = {
            LIST: [{ CODE: "EQUAL", NAME: this.$t("equal") }],
            DATE: [{ CODE: "BETWEEN", NAME: this.$t("between") }],
            STRING: [
          ...
          try {
            this.columnNameList = this.columns
              .filter((q) => q["datafield"] != "_rowstatus" && q["datafield"] != "_rowstatusUnMarked")
              .map((q) => {
                return { CODE: q["datafield"], NAME: q["caption"] };
              });
      ...
      addMoreFilter(idx) {
        try {
          ...
          let addedCols = this.gridFilterList.map((q) => {
            return q["column"];
          });
      ...
      async exportExceljs(excelHeader, exportData) {
        ...
        try {
          fileExportName = document.getElementById("appBar").getElementsByClassName("v-chip--active")[0].getElementsByClassName("v-chip__content")[0].innerText;
        } catch (error) {
      ...
      myMenuSettingOnItemClick(event) {
        ...
        switch (args.id) {
          ...
          case "resize_column":
            ...
            break;
          case "column_display":
            this.$refs.columnDisplay.dialogIsShow = true;
            break;
          case "checkbox": {
            this.gridSelectionMode = "checkbox";
            this.rebuildHeader();
      ...
      onSaveHeader() {
        //save cookie
        let records = this.$refs.myGrid.columns.records
          .filter((q) => !this.sysCols.includes(q.datafield))
          .map((q, idx) => {
        ...
        if (objs instanceof Array) {
          for (let idx = 0; idx < objs.length; idx++) {
                    ...
                    arrayAggregate.forEach((e, i) => {
                      switch (e) {
                        ...
                        case "max":
                          ...
                          break;
                        case "avg":
                          aggregatesName = "average (x)";
                          break;
                        case "range":
                          aggregatesName = "range (r)";
                          break;
                        case "stdev":
                          aggregatesName = "stdev.s (σ)";
                          break;
                        case "r/x":
                          aggregatesName = "R/X (%)";
                          break;
                      ...
                      if (i % 2 == 1) {
                        headerStr =
                          headerStr +
                          `<div style="background-color: #eef09d;width:100%;height:auto;border-bottom:0.5px solid #a7a7a7"">
                                                      <span style="color:red"><b>${that.$t(aggregatesName)}</b></span>
                          </div>`;
                      } else {
                        headerStr =
                          headerStr +
                          `<div style="background-color: #f6f7cb;width:100%;height:auto;border-bottom:0.5px solid #a7a7a7"">
                                                      <span style="color:red"><b>${that.$t(aggregatesName)}</b></span>
                          </div>`;
                      }
                    });
            ...
            if (entry.hasOwnProperty("columns")) {
              ...
              try {
                entry.columntype = entry.dataType;
              } catch {}
              ...
              keys.forEach((key) => {
                switch (key) {
                  case "dataField": {
                    entry.datafield = entry[key];
                    break;
                  }
                  case "alignment": {
                    entry.cellsalign = entry[key];
                    break;
                  }
                  case "visible": {
                    entry.hidden = !entry[key];
                    break;
                  }
                  case "caption": {
      ...
      async loadData(p_filter_paras = null) {
        ...
        if (this.autoresize) {
          ...
          loadDataDBCommon(filter_paras) {
              this.isProcess = true;
              if (filter_paras) {
                  this._onLoadDBCommon(filter_paras);
              } else {
                  setTimeout(() => {
                      this._onLoadDBCommon(this.filter_paras);
                  }, 200);
              }
          },
      ...
      async _onLoad(p_filter_paras) {
        if (this.sel_procedure) {
          ...
          async _onLoadDBCommon(filter_paras) {
              // console.log("_onLoadDBCommon!")
              if (this.sel_procedure) {
                  let dso = {
                      type: "grid",
                      selpro: this.sel_procedure,
      ...
      myGridOnFilter: function (event) {
        try {
          this.rowCount(this.$refs.myGrid.getdisplayrows().length);
        } catch {}
      ...
      autoMakeSEQ(p_datafield, startIndex = 1) {
        ...
        for (let i = 0; i < rowsdata.length; i++) {
          ...
          rowsdata[i]._rowstatus = rowsdata[i]._rowstatus !== "i" && rowsdata[i]._rowstatus !== "d" ? "u" : rowsdata[i]._rowstatus;
        }
      ...
      setCellValue(p_datafield, p_value, p_pk, p_field_name = "PK") {
        ...
        if (idx >= 0) {
          ...
          rowsdata[idx]._rowstatus = rowsdata[idx]._rowstatus !== "i" && rowsdata[idx]._rowstatus !== "d" ? "u" : rowsdata[idx]._rowstatus;
          this.setDataSource(rowsdata);
          this.gridUpdate();
        }
      },
      ...
      async _rebuildHeader(headerType = 0) {
        if (this.setting != null) {
          //vng-207 hien cai dong setting cho grid hr
      ...
      buildColumn(keys, headers, level = 0) {
        ...
        if (isLast) {
          ...
          keys.forEach((key) => {
            switch (key) {
              case "title": {
                column.caption = this.autotranslate && !!!headers["isAdditionColumn"] ? this.$t(headers[key].toLowerCase()) : String(headers[key]);
                break;
              }
              case "fixed": {
                column.pinned = headers[key];
                break;
              }
              case "hidden": {
              ...
              case "field": {
                column.dataField = headers[key];
                break;
              }
              case "editable": {
                column.allowEditing = headers[key];
                break;
              }
              case "type": {
      ...
      onSetAll(column, value, isUpdateStatus) {
        ...
        for (let idx = 0; idx < this.$refs.myGrid.getrows().length; idx++) {
          ...
          this.$emit("cell-changed", { rowindex: idx, datafield: column, oldvalue: oldValue, value: value, row: this.$refs.myGrid.getrowdata(idx) }, this._buildDataSaveDb(this.$refs.myGrid.getrowdata(idx)));
        }
        return true;
      },
      ...
      onAdd(data, isLastRow = false, p_position = 0) {
        ...
        try {
          this.rowCount(this.$refs.myGrid.getrows().length);
        } catch {}
      },
      ...
      async onPrintReport() {
        ...
        if (result) {
          if (result.some((q) => q["USE_GRID_DATA"] == "Y")) {
            ...
            dataCombobox = headerTmp.map((q) => {
              return { datafield: q["datafield"], lookup: q["lookup"] };
            });
        ...
        if (setCol != "" && setCol != null) {
          ...
          if (type != "DIALOG") {
            ...
            try {
              ...
              if (!this.gridSetColDialogVal || !this.dialogCallback) {
                //ko chon thi set lai null
