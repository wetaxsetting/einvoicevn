  ...
  <v-container fluid class="px-0 pt-0" v-resize="onResize" >
    ...
    <v-row  class="px-2">
        <v-col cols="12" lg="6" >
            <v-row no-gutters>
                <v-col cols="12" lg="11">
                    <v-text-field clearable dense hide-details outlined  :label="$t('name_id')" v-model="searchText" @keypress.enter="onSearchUser"></v-text-field>
                </v-col>
                <v-col cols="12" lg="1" align="right">
                    <v-row no-gutters>
                        <v-spacer></v-spacer>
                        <BaseButton btn_type="icon" icon_type="save" :btn_text="$t('save')"  @onclick="onProcessConfirm('SAVE_USER')" />
                    </v-row>
                   
                </v-col>
            </v-row>
            
            <BaseGridView :headertype="1" ref='idGrid' :height="limitHeight"  :header="header1"  @cellClick="onSearchOrg" ></BaseGridView>
        </v-col>
        <v-col cols="12" lg="6" align="right" >
            <v-row no-gutters>
                <v-spacer></v-spacer>
                <BaseButton btn_type="icon" icon_type="save" :btn_text="$t('save')"  @onclick="onProcessConfirm('SAVE_ORG')" />
            </v-row>
            
            <BaseGridView :headertype="1" ref='idGrid2' :height="limitHeight2"  :header="header2" :editable="true" ></BaseGridView>
        </v-col>
    </v-row>
