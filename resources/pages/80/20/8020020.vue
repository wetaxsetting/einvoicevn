              ...
              <v-container fluid class="py-1">
                <v-row dense align="center" justify="space-between">
                  ...
                  <v-col lg="3" sm="6" cols="12">
                    <v-select cache-items clearable dense hide-details  item-value="PK" item-text="TEXT" :label="$t('company', 'common')" :items="companyList" v-model="lstCompany" @change="onBizPlace"></v-select> 
                  </v-col>
                  <!-- Customer  -->
                  ...
                  <!-- Biz Place -->
                  <v-col lg="3" cols="12">
                       <v-select dense cache-items hide-details  item-value="PK" item-text="TEXT" :label="$t('biz_place')" :items="bizPlaceList" v-model="lstBiz_Place" ></v-select>
                  </v-col>  
                  <!-- Type  -->
                    <v-col lg="2" cols="12">
                        <v-select dense cache-items hide-details  item-value="PK" item-text="TEXT" :label="$t('voucher_type')" :items="typeList" v-model="lstType" ></v-select>
                    </v-col>                 
                  <!-- Transaction -->
                    <v-col lg="2" cols="12">
                        <v-select dense cache-items hide-details  item-value="PK" item-text="TEXT" :label="$t('transaction')" :items="transactionList" v-model="lstTransaction" ></v-select>
                    </v-col>
                  <!-- Use Y/N -->
                  <v-col lg="1" cols="12">
                      <v-select dense cache-items hide-details  item-value="PK" item-text="TEXT" :label="$t('use_yn')" :items="useYNList" v-model="lstUseYN" ></v-select>
                  </v-col>
                  <!-- Interface Y/N -->
                  <v-col lg="1" cols="12" class="py-1">
                            <v-select dense cache-items hide-details  item-value="PK" item-text="TEXT" :label="$t('interface_yn')" :items="interfaceYNList" v-model="lstInterfaceYN" @change="onChangeButton"></v-select>
                  </v-col> 
                  <!-- row 3 -->
                  <v-col lg="1" cols="12">
...
export default {
    ...
    deInquiryHeaders() {
      ...
      return [
        ...
        },
        {
          ...
          {
          dataField:"PARTNER_LNAME",  allowEditing:"false", 
  ...
  methods: {
    async getListCodes() {
      ...
      const dso_bizplace_list = { type: 'list', selpro: 'SYS_SEL_BIZ_PLACE', para: [ this.lstCompany ] }
      this.bizPlaceList = await this._dsoCall(dso_bizplace_list, 'select', false) 
      this.lstBiz_Place = this.bizPlaceList[0].PK

      this.lstUseYN = 'Y'

      const dso_invoice_type = { type: 'list', selpro: 'AC_GET_COMMONCODE_EI', para: [this.lstCompany,'TPS0023' ] }
    ...
    onUpdatePassWordMd5()
    {
      //console.log(this.dxg_gridMaster.length);
    ...
    async insertStatusPartner(){
      console.log(`Partner: `+this.txtPartnerPk);
      const dso = {
        type: "process",
        updpro: "SP_PRO_8020020_BUSPARTNER",
        para: [
               this.txtPartnerPk,
