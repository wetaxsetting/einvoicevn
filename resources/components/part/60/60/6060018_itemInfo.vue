      ...
      <v-col cols="12">
        ...
        <v-row align="center" justify="center">
          ...
          <DataGridView column-resizing-mode="widget" 
              ref="grdItemInfo_N" 
              :auto_load="false" select_mode="Multiple"
              ...
              sel_procedure="AC_SEL_6060018_CRCAD_OTHER_NC"
              upd_procedure="AC_UPD_6060018_TAC_CRCAD_OTHER" 
              @callSaveResult="afterSave"
              :filter_paras="[txtCrca_Pk]" 
              :update_paras="['PK',    
                              'TAC_CRCA_OTHER_PK',     
                              'ORDER_NO',              
                              'TCO_ITEM_PK',           
...
export default {
  ...
  methods: 
  {
    onSelectItemInfo(item)
      {
        //var selectRows = this.$refs.grdItemInfo_N.getSelectRows();
        if(item.length == 1)
        {
            ...
            for (let i = 0; i < item.length; i++) {
              ...
              }else
              {
                this.$refs.grdItemInfo_N.addRowStruct({
                      ...
                      TAC_CRCA_OTHER_PK : this.txtCrca_Pk,
    ...
    onSelectItemInfoTO(item){
            ...
            {
                //const itemAccIdx = this.itemInfoList.length
                //this.itemInfoList[itemAccIdx]._rowstatus = "i"
                for(let i = 0; i < item.length; i++){
                  this.itemInfoList.unshift({
                    _rowstatus: "i",
                    PK: "",
                    TCO_ITEM_PK: item[i].PK,
                    ITEM_CODE:item[i].ITEM_CODE,
                    ...
                    WH_NAME:item[i].WH_NAME,
                    TIN_WAREHOUSE_PK:item[i].TLG_IN_STORAGE_PK,
                    SEQ:this.itemInfoList.length+1,
                    SEQ_DIS:this.itemInfoList.length+1
            ...
            if(!this.itemInfoList[itemAccIdx]._rowstatus){
              ...
              {
                this.itemInfoList[itemAccIdx]._rowstatus = "u"  
                this.itemInfoList[itemAccIdx].TCO_ITEM_PK = item[0].PK
                this.itemInfoList[itemAccIdx].ITEM_CODE = item[0].ITEM_CODE
                this.itemInfoList[itemAccIdx].ITEM_NAME = item[0].ITEM_NAME
                this.itemInfoList[itemAccIdx].WH_NAME = item[0].WH_NAME
                this.itemInfoList[itemAccIdx].TIN_WAREHOUSE_PK = item[0].TLG_IN_STORAGE_PK
              } 
            } 
          }  
      }   
  ...
  async  openDialog(row) {
    ...
    async calculateCellExtPrice(data) {
      if (data)
      {
        ...
        {
              proc: "AC_SEL_6060018_AMOUNT",
              para: [this.txtCrca_Pk]
        })
        .then((res) => {
            if (res.data) {
                const value = res.data[0]
                this.txtTotal_Trans = this._formatNumber(value.NET_TR_AMT,2);
    ...
    itemInfoHeader_N() {
      ...
      this.headerGrid = [				
        ...
        {
          dataField:"TAC_CRCA_OTHER_PK", width:"0",
          allowEditing:false,
          caption: this.$t('tac_crca_other_pk'),
          visible: false
        },
        {
          dataField:"ORDER_NO",  
        ...
        {
          dataField:"HS_CODE",  width: 0,visible: false,
        ...
        {
          dataField:"TR_AMT", allowEditing:true,
          ...
          dataField:"BK_AMT",
          width:120,visible: false,
          caption: this.$t('bk_amt'), allowEditing:true,
          ...
          caption: this.$t('order_uprice'), allowEditing:true,
           width: 0,
            customizeText: function({ value }) {
                 ...
                 {
                     return `${self._formatNumber(value, 2)}`;//  return value.toFixed(4);//return `${self._formatNumber(value, 4)}`;
                 }
        ...
        },
         {
          ...
          caption: this.$t('vat_tr_amt'), allowEditing:true,
             width: 0,
              customizeText: function({ value }) {
                 if(self.txtTrans_Ccy ==='VND'){
                     return `${self._formatNumber(value, 0)}`;
        ...
        },
         {
          ...
          caption: this.$t('vat_bk_amt'), allowEditing:true,
             width: 0,
              customizeText: function({ value }) {
                 if(self.txtTrans_Ccy ==='VND'){
                     return `${self._formatNumber(value, 0)}`;
        ...
        },
        {
          ...
          caption: this.$t('vat_tax_amt'), allowEditing:true,
             width: 0,
              customizeText: function({ value }) {
                 if(self.txtTrans_Ccy ==='VND'){
                     return `${self._formatNumber(value, 0)}`;
