<template>
  <v-container fluid class="pa-0" v-resize="onResize">
      ...
      <v-col cols="12">
        ...
        <v-row align="center" justify="center">
          <v-col lg="2" cols="12">
            ...
            <v-text-field clearable dense hide-details readonly :label="$t('account_cd')" @dblclick="openAccountDialog"
              v-model="txtTRANS_CR_CDS"></v-text-field>
          </v-col>
          <v-col lg="1" cols="12" class="text-left">
            <v-tooltip>
              <template v-slot:activator="{ on }">
                <v-btn icon tile :color="currentTheme" :disabled="isProcessing" @click="onFill" v-on="on">
          ...
          <v-col lg="2" cols="12">
            <v-text-field clearable dense hide-details :label="$t('total_net_amount')" v-model="txtTotal_Trans"
              format="###,###,###.##" v-show="true"></v-text-field>
          </v-col>
          <v-col lg="2" cols="12">
            <v-text-field clearable dense hide-details :label="$t('total_book_amount')" v-model="txtTotal_Book"
              format="###,###,###.##" v-show="true"></v-text-field>
          </v-col>
          <v-col lg="1" cols="12">
            <v-select dense cache-items hide-details item-value="PK" item-text="TEXT" :label="$t('get_data_type')"
              :items="getDataList" v-model="lstGetData"></v-select>
          </v-col>
          <v-col lg="4" cols="12" class="text-right">

            <div class="d-flex justify-end">
              <!-- Add -->
              <BaseButton icon_type="add_new" :btn_text="$t('btn_add')" :disabled="!btnNew_D"
                @onclick="onAddAcntMaster()" />

              <!-- Save -->
              <BaseButton icon_type="save" :btn_text="$t('save')" :disabled="!btnSave_D" @onclick="onSaveMaster()" />

              <!-- Delete -->
              <BaseButton icon_type="delete" :btn_text="$t('delete')" :disabled="!btnDel_D"
                @onclick="onDeleteAcntMaster()" />

            </div>
          </v-col>
          <v-col cols="12" class="py-0">
            <DataGridView column-resizing-mode="widget" ref="grdItemInfo_N" :auto_load="false" select_mode="Single"
              :max_height="limitHeight" :header="headerGrid" @cellDblClick="openDialog" :onCellPrepared="onCellPrepared"
              @row-updated="checkUpdatedItem" @callSaveResult="afterSave" @setDataSource="onAfterLoad"
              @selection-changed="selectionChanged" sel_procedure="AC_SEL_6060010_TAC_CRCAD_NC"
              upd_procedure="AC_UPD_6060010_TAC_CRCAD" :filter_paras="[this.txtCrca_Pk]" :update_paras="['PK',
                  'TAC_CRCA_PK',
                  'ORDER_NO',
                  'TCO_ITEM_PK',
                  'ITEM_UOM',
    ...
    <delete-dialog ref="deleteDialogMaster" :deleteProps="deletePropsMaster"
      @returnDeletedItem="removeDeletedItemMaster"></delete-dialog>
    ...
    <get-data-o-dialog ref="getDataODialog" @callBackData="onSelectItemInfoTO"
      :customer="{ PK: txtCustomer_PK, NAME: txtCustomer_CD }"></get-data-o-dialog>
    ...
    <GetDataAARDialog ref="refGetDataAAR" @callBackData="getDataAAR" :multiSelectMode="true" :hideSelect="false"
      :companyPK="this.lstCompany" :bizPlacePK="this.lstBizPlace" :vendorPK="this.txtCustomer_PK"
      :vendorNM="this.txtCustomer_CD" :trans_dt="this.txtTRANS_DT" />
    <dynamic-dialog ref="refDynamicDialog" :companyPK="user.TCO_COMPANY_PK" :header="dynamicHeader" :listLabel="listLabel"
      :listData="listData" :codeLabel="codeLabel" :nameLabel="nameLabel" :dialogTitle="dialogTitle" :procedure="procedure"
      :moreParas="moreParas" :autoSearch="autoSearch" :multiSelectMode="multiSelectMode"
...
import GetDataAARDialog from '@/components/dialog/GetDataAARDialog';
export default {
  layout: "default",
  middleware: "user",
  ...
  computed: {
    ...
    limitHeight() {
      return Math.floor(this.formContainerHeight * 0.6) - 150;
    },
    limitWidth() {
  ...
  watch: {
    paras(val) {
      ...
      if (val.length > 0) {
        ...
        this.txtTRANS_DT = val[19];
        this.lstBizPlace = val[20];
        this.buttonStatus();
  ...
  created() {
    ...
    if (this.paras !== "" || this.paras !== null || this.paras !== undefined) {
      if (this.paras.length > 0) {
        ...
        this.txtTRANS_DT = this.paras[19];
        this.lstBizPlace = this.paras[20];
  ...
  methods:
  {
    onSelectItemInfo(item) {
      if (item.length == 1) {
        ...
        this.$refs.grdItemInfo_N.setCellSelected("INV_UOM", item[0].UOM)
      } else {
    ...
    getDataAAR(item) {
      ...
      for (let i = 0; i < item.length; i++) {
        this.$refs.grdItemInfo_N.addRowStruct({
          ...
          U_PRICE: item[i].UNIT_PRICE,
          QTY: item[i].QTY,
          EXT_PRICE: item[i].EXT_PRICE,
          NET_TR_AMT: item[i].TR_AMT,
          NET_BK_AMT: item[i].BK_AMT,
          ORDER_UPRICE: 0,
          ORDER_TRAMT: 0,
          TAC_ABACCTCODE_PK: this.txtTRANS_CR_PK,
          AC_CD: this.txtTRANS_CR_CD,
          ...
          TAC_CRCA_PK: this.txtCrca_Pk,
          WH_NAME: item[i].WH_NAME,
          TIN_WAREHOUSE_PK: item[i].TIN_WAREHOUSE_PK,
          SEQ: this.$refs.grdItemInfo_N.dataList.length + 1,
          SEQ_DIS: this.$refs.grdItemInfo_N.dataList.length + 1,
          ATTRIBUTE2: item[i].ITEM_NAME,
          TLG_CS_DEBITNOTE_SV_PK: item[i].TLG_CS_DEBITNOTE_M_PK,
    ...
    async getListCodes() {
      ...
      if (results.length) {
        this.listVAT = results[0]; //ACCR0110

        this.listNature = results[1]; //ACEIS100
      }
    },
    ...
    afterSave(data) {
      if (data) {
        ...
        //     RESEARCH_YN : "Y"
        //   }
        //   this.$emit('retunData', dataReturn );
    ...
    onAfterLoad() {
      let total_tr_amt = 0, total_bk_amt = 0, total_bk_amt_dis = 0, total_tr_amt_dis = 0, total_ext_amt_dis = 0;
      // console.log("onAfterLoad");
      for (var i = 0; i < this.$refs.grdItemInfo_N.getDataSource().length; i++) {

        total_ext_amt_dis = (this.$refs.grdItemInfo_N.getDataSource()[i].EXT_PRICE);
        total_tr_amt_dis = (this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT);
        total_bk_amt_dis = (this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT);
        // console.log("onAfterLoad  ==>> total_bk_amt_dis :",total_bk_amt_dis , " total_tr_amt_dis: ", total_tr_amt_dis , "total_bk_amt_dis : ", total_bk_amt_dis);
        if (this.txtTrans_Ccy === 'VND') {

          this.$refs.grdItemInfo_N.getDataSource()[i].EXT_PRICE = total_ext_amt_dis == null ? "" :  Number(total_ext_amt_dis).toFixed(0);
          this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT = total_tr_amt_dis == null ? "" : Number(total_tr_amt_dis).toFixed(0);
          this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_TRAMT = total_tr_amt_dis == null ? "" : Number(total_tr_amt_dis).toFixed(0);
          this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_UPRICE = total_tr_amt_dis == null ? "" : Number(total_tr_amt_dis).toFixed(0);

        } else {

          //this.txtTotal_Trans = this._formatNumber(total_tr_amt,2);
          //this.txtTotal_Book  = this._formatNumber(total_bk_amt,0); 

          this.$refs.grdItemInfo_N.getDataSource()[i].EXT_PRICE = total_ext_amt_dis == null ? "" :  Number(total_ext_amt_dis).toFixed(2);
          this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT = total_tr_amt_dis == null ? "" :  Number(total_tr_amt_dis).toFixed(2);
          this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_TRAMT = total_tr_amt_dis == null ? "" :  Number(total_tr_amt_dis).toFixed(2);
          this.$refs.grdItemInfo_N.getDataSource()[i].ORDER_UPRICE = total_tr_amt_dis == null ? "" :  Number(total_tr_amt_dis).toFixed(2);

        }
        console.log("txtTrans_Rate +++> ", this.txtTrans_Rate);
        this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT = total_bk_amt_dis == null ? "" : Number(total_bk_amt_dis).toFixed(0);
    ...
    onCellPrepared({ column, cellElement, rowType }) {
      if (this.txtInvoice_type === "E" || this.txtInvoice_type === "C") {
    ...
    onAddAcntMaster(item) {
      ...
      else if (this.lstGetData === "AAR") {
        this.$refs.refGetDataAAR.dialogIsShow = true;
      }
    },
    ...
    checkUpdatedItem(data, colName) {
      ...
      if (colName === 'SEQ_DIS') {
        ...
        if(data.data.SEQ_DIS == "" || data.data.SEQ_DIS == null)
        {
          data.data.ATTRIBUTE5 = "4";
        }
      }
      if (colName === 'NET_TR_AMT') {
        let v_tran_amt = 0;
        v_tran_amt = data.data.NET_TR_AMT;
        if (this.txtTrans_Ccy === 'VND') {
          data.data.NET_BK_AMT = data.data.NET_TR_AMT;
        } else {
          data.data.NET_BK_AMT = Number(v_tran_amt).toFixed(2) * this.txtTrans_Rate / this.txtBook_Rate;
        }

      }
      if (colName === 'U_PRICE' || colName === 'QTY') {
        let v_tr_amt = 0, v_bk_amt = 0, v_exp_amt = 0, v_vat_rate = 0, v_vat_tr_amt = 0, v_vat_bk_amt = 0;
        if (this.txtVat_Tax == '10X0.70') {
          v_vat_rate = 7;
        }
        else if (this.txtVat_Tax == '01') {
          v_vat_rate = 0;
        } else {
          v_vat_rate = Number(this.txtVat_Tax);
        }
        v_exp_amt = data.data.QTY * data.data.U_PRICE;//data.data.EXT_PRICE;//  data.data.QTY*data.data.U_PRICE;
        v_tr_amt = data.data.QTY * data.data.U_PRICE;
        v_bk_amt = Number(v_tr_amt.toFixed(2)) * this.txtTrans_Rate / this.txtBook_Rate;
   
        v_vat_bk_amt = Number(v_bk_amt) * Number(v_vat_rate) /100; 
        v_vat_tr_amt = Number(v_tr_amt) * Number(v_vat_rate) /100;

        if (this.txtTrans_Ccy === 'VND') {
          data.data.EXT_PRICE = Number(v_exp_amt).toFixed(0);
          data.data.NET_TR_AMT = Number(v_tr_amt).toFixed(0);
          data.data.NET_BK_AMT = Number(v_tr_amt).toFixed(0);   // vì tiền TRANS = BOOK v_bk_amt
          data.data.ORDER_TRAMT = Number(v_tr_amt).toFixed(0);
          data.data.ORDER_UPRICE = Number(v_tr_amt).toFixed(0);

          data.data.VAT_TR_AMT = Number(v_vat_tr_amt).toFixed(0);
          data.data.VAT_BK_AMT = Number(v_vat_bk_amt).toFixed(0);
          //console.log("v_tr_amt.toFixed(2)  : ", v_tr_amt.toFixed(2) + " ,this.txtTrans_Rate  " + this.txtTrans_Rate + " ,this.txtBook_Rate  " + this.txtBook_Rate); 
          //console.log("v_tr_amt  : ", Number(v_tr_amt).toFixed(0) );
          //console.log("v_bk_amt  : ", Number(v_bk_amt).toFixed(0) );

        }
        else {
          data.data.EXT_PRICE = Number(v_exp_amt).toFixed(2);
          data.data.NET_TR_AMT = Number(v_tr_amt).toFixed(2);
          data.data.NET_BK_AMT = Number(v_bk_amt).toFixed(0);
          data.data.ORDER_TRAMT = Number(v_bk_amt).toFixed(0);
          data.data.ORDER_UPRICE = Number(v_bk_amt).toFixed(0);
          data.data.VAT_TR_AMT = Number(v_vat_tr_amt).toFixed(2);
          data.data.VAT_BK_AMT = Number(v_vat_bk_amt).toFixed(0);
        }
      }
      this.calculateCellExtPrice();
    ...
    async calculateCellExtPrice() {
      if (this.txtCrca_Pk > 0) {
        let total_tr_amt = 0, total_bk_amt = 0, total_vat_bk_amt = 0, total_vat_tr_amt = 0, v_vat_rate = 0;
        if (this.txtVat_Tax == '10X0.70') {
          v_vat_rate = 7;
        }
        else if (this.txtVat_Tax == '01') {
        ...
        if (this.$refs.grdItemInfo_N.getDataSource().length) {
          for (var i = 0; i < this.$refs.grdItemInfo_N.getDataSource().length; i++) {
            let bk_amt = 0, tr_amt = 0, vat_bk_amt = 0, vat_tr_amt = 0;

            tr_amt = Number(this.$refs.grdItemInfo_N.getDataSource()[i].NET_TR_AMT);
            bk_amt = Number(this.$refs.grdItemInfo_N.getDataSource()[i].NET_BK_AMT);

            // vat_bk_amt = Number(bk_amt) * Number(v_vat_rate) /100; 
            // /vat_tr_amt = Number(tr_amt) * Number(v_vat_rate) /100;

            vat_bk_amt = Number(this.$refs.grdItemInfo_N.getDataSource()[i].VAT_BK_AMT);
            vat_tr_amt = Number(this.$refs.grdItemInfo_N.getDataSource()[i].VAT_TR_AMT);
            // console.log(vat_tr_amt + "   " + vat_tr_amt);
            if (this.txtTrans_Ccy === 'VND') {
              total_tr_amt += Number(tr_amt.toFixed(0));
              total_bk_amt += Number(bk_amt.toFixed(0));
              total_vat_bk_amt += Number(vat_bk_amt.toFixed(0));
              total_vat_tr_amt += Number(vat_tr_amt.toFixed(0));
            } else {
              //console.log("vat_tr_amt  " +  vat_tr_amt   + " v_vat_rate  " + vat_tr_amt.toFixed(2))

              total_bk_amt += Number(bk_amt.toFixed(0));
              total_tr_amt += Number(tr_amt.toFixed(2));
              total_vat_bk_amt += Number(vat_bk_amt.toFixed(0));
              total_vat_tr_amt += Number(vat_tr_amt.toFixed(2));
            }

          }

          if (this.txtTrans_Ccy === 'VND') {
            this.txtTotal_Trans = this._formatNumber(total_tr_amt, 0);
            this.txtTotal_Book = this._formatNumber(total_bk_amt, 0);

          } else {
            // console.log("total_bk_amt " + total_bk_amt);
            this.txtTotal_Trans = this._formatNumber(total_tr_amt, 2);
            this.txtTotal_Book = this._formatNumber(total_bk_amt, 0);

            // console.log("this.txtTotal_Book " + this.txtTotal_Book);
          }
          // console.log("this.$refs.grdItemInfo_N.getDataSource().length  " + this.$refs.grdItemInfo_N.getDataSource().length0);
          // if(this.$refs.grdItemInfo_N.getDataSource().length == 0)
          // {
          ...
          let dataReturn = {
            PK: this.txtCrca_Pk,
            TRAN_AMT: total_tr_amt,
            BOOK_ANT: total_bk_amt,
            TOT_VAT_TRAN_AMT: total_vat_tr_amt,
            TOT_VAT_BOOK_AMT: total_vat_bk_amt,
            TOT_TRAN_AMT: total_tr_amt + total_vat_tr_amt,
            TOT_BOOK_ANT: total_bk_amt + total_vat_bk_amt,
            RESEARCH_YN: "N"
          }
          this.$emit('retunData', dataReturn);
    ...
    itemInfoHeader_N() {
      ...
      this.headerGrid = [
        ...
        },
        {
          ...
          allowEditing: true,
          dataType: 'number',
          formatFloat: self.txtTrans_Ccy == 'VND' ? 5 : 4,
          //    customizeText: function({ value }) {
        ...
        },
        {
          ...
          allowEditing: true,
          dataType: 'number',
          formatFloat: self.txtTrans_Ccy == 'VND' ? 0 : 2,
          //   customizeText: function({ value }) {
          //      if(self.txtTrans_Ccy === `VND`){
        ...
        },
        {
          ...
          visible: false,
          dataType: 'number',
          formatFloat: self.txtTrans_Ccy == 'VND' ? 0 : 4,
          // customizeText: function({ value }) {
          //      if(self.txtTrans_Ccy ==='VND'){
          //          return `${self._formatNumber(value, 0)}`;
          //      }else
          //      {
          //          return `${self._formatNumber(value, 4)}`;
          //      }
        ...
        {
          dataField: "ATTRIBUTE2", width: 200,
        ...
        {
          dataField: "ATTRIBUTE5", width: 150,
          caption: this.$t('attribute5'),
          lookup: {
            displayExpr: "NAME",
            valueExpr: "CODE",
            dataSource: this.listNature
          },
          cssClass: "cell-align-center"
        },
        {
        ...
        {
          dataField: "INV_UOM", width: 80,
          caption: this.$t('inv_item_uom'),
          lookup: {
            displayExpr: "NAME",
            valueExpr: "NAME",
            dataSource: this.listUOM
        ...
        {
          dataField: "INV_QTY", allowEditing: true,
          width: 80,
