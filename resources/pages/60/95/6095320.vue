    ...
    <v-row no-gutters class="pl-2">
      <v-col v-show="showLeft" :cols="leftCols">
        <v-card class="pa-2">
          ...
          <v-row>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('serial_no')" v-model="SERIAL_NO"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('invoice_no')" v-model="INVOICE_NO"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('Seller_code')" v-model="seller_no"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('Seller_name')" v-model="seller_name"></v-text-field>
            </v-col>
            <v-col md="2">
              <BaseSelect :label="$t('form_no')" v-model="selected_form_no" :lstData="form_no_list" item-text="NAME" item-value="VAL" />
            </v-col>
            <v-col md="2">
              <BaseSelect :label="$t('ei_status')" v-model="selected_status" :lstData="status_list" item-text="NAME" item-value="VAL" />
            </v-col>
          </v-row>
          <v-row>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('mtc')" v-model="matracuu"></v-text-field>
            </v-col>
            <!-- <v-col sm="3" md="3">
              <v-file-input
                ref="refFile"
            ...
            <v-col md="3">
              <v-text-field clearable dense hide-details :label="$t('website')" v-model="WEB_SITE"></v-text-field>
            </v-col>
            <v-col md="3">
              <v-text-field clearable dense hide-details :label="$t('mcqt')" v-model="macqt"></v-text-field>
            </v-col>
          </v-row>
          <v-row>
            <v-col>
      ...
      <v-col class="pa-1" :cols="12 - leftCols">
        <v-card class="pa-1">
          <v-row>
            ...
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('item_serial_no')" v-model="item_serial_no"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('item_invoice_no')" v-model="item_invoice_no"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('invoice_date')" v-model="item_invoice_date"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('sale_taxcode')" v-model="item_sale_taxcode"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('sale_company_name')" v-model="item_sale_company_name"></v-text-field>
            </v-col>
          </v-row>
          <v-row>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('sale_address')" v-model="item_sale_address"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('amt_exclude_vat')" v-model="item_exclude_vat"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('amt_vat')" v-model="item_amt_vat"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('amt_include_vat')" v-model="item_include_vat"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('so_hoa_don')" v-model="item_so_hoa_don"></v-text-field>
            </v-col>
            <v-col md="2">
              <v-text-field clearable dense hide-details :label="$t('ngay_lap_hoa_son')" v-model="item_nlhd"></v-text-field>
            </v-col>
          </v-row>
  ...
  methods: {
    ...
    onDownloadXml() {
      for (var i = 0; i < this.selected_rows.length; i++) {
        ...
        if (txtPK != '') {
          var url = 'http://csharp-api.webcashvietnam.com/wseinvoice/BSService.asmx/Download_File_XML_v4?pk=' + txtPK;
    ...
    async changeFile(file) {
      ...
      if (!this.selected_company) {
        return this.showNotification('warning', this.$t('please_select_your_company_firstly'), '', 5000);
      }
      if (this.file.type == 'application/pdf') {
        if (!this.tei_einvoice_cloud_pk) {
          return this.showNotification('warning', this.$t('please_upload_file_xml_firstly'), '', 5000);
        }
      }
    ...
    async onClickButton() {
      if (this.pdfUrlRow == '') {
        this.showNotification('warning', this.$t('please_insert_pdf_file_first'), '', 5000);
      } else {
        const res = await this.$axios.$get('/dso/getfile?file_name=' + this.pdfUrlRow + '', {
          responseType: 'blob',
        });
