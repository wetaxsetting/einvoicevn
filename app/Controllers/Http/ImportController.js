...
class ImportController {
    ...
    try {
      ...
      if (file_path && file_path.indexOf('.xml') > 0) {
        ...
        const result = await DBService.callProcCursor(
          'ei_upd_tei_einvoice_cloud_pdf_file',
          [tei_einvoice_cloud_pk, pdfRelativePath],
          p_language,
          p_crt_by,
        );
  ...
  async extractXMLContentBillHaiPhongPort(p_xml_path, p_language, p_crt_by, file_name) {
    try {
      ...
      if (xml_encoding == 'UTF16') {
        xml_encoding = 'UTF16LE';
      }
      let AESs = new AES();
      ...
      let masterPara = [
        '',
        '',
        jsonInvoiceData[0].templateCode,
        jsonInvoiceData[0].invoiceSeries,
        jsonInvoiceData[0].invoiceNumber,
        jsonInvoiceData[0].invoiceAppRecordId,
        jsonInvoiceData[0].invoiceIssuedDate,
        '',
        '',
      ...
      if (master && master[0].PK > 0) {
        ...
        for (let i = 0; i < jsonItems.length; i++) {
          const detailPara = [
            master[0].PK,
            '1',
            jsonItems[i].lineNumber,
            jsonItems[i].itemCode,
            jsonItems[i].itemName,
            jsonItems[i].unitName,
            jsonItems[i].quantity,
            jsonItems[i].unitPrice,
            jsonItems[i].itemTotalAmountWithoutVat,
            '0',
            '0',
            '0',
          ];
  ...
  async UploadFileToFolderTKHQ({request, response, auth}) {
    ...
    try {
      ...
      if (file_path && file_path.indexOf('.xml') > 0) {
        ...
        const pdfRelativePath = file_path.replace(ROOT_DIR_FILES, '');
        const result = await DBService.callProcCursor(
          'ei_upd_tei_ecus_declare_pdf_file',
          [tei_ecus_declare_pk, pdfRelativePath],
          p_language,
          p_crt_by,
        );
        return response.send(Utils.response(true, 'upload_pdf_file_sucessfull', null));
  ...
  async ImportExcelFileRPA({request, response, auth}) {
    try {
      ...
      if (result) {
        ...
        // let resultData = await imp.writeFile();
        if (
          file_name_check == 'Credit_Confirm_VND' ||
          file_name_check == 'Credit_Confirm_USD' ||
          file_name_check == 'Debit_Confirm_VND' ||
          file_name_check == 'Debit_Confirm_USD'
        ) {
          const para_file = {
