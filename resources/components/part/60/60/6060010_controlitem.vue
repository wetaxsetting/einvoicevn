  ...
  <v-container fluid class="pa-0" v-resize="onResize">
    <v-row no-gutters>
        ...
        <v-col lg="12"  cols="12" class="text-right">
            ...
            <v-btn icon tile :color="currentTheme"   :disabled="!btnSave_More"  @click="onSaveMaster" >
    ...
    <ctrl-apply-invoice-dialog ref="ctrlapplyinvoice" @returnApplyInvoiceInfo="returnApplyInvoiceInfo" :company="this.selectedCompany" :accountItem="accountItem" :partnerItem="PartnerItem" :date="dateItem" :ccy="getCcy"></ctrl-apply-invoice-dialog>
...
export default {
  ...
  components: {
    ...
    // accountDialog: () => import("@/components/dialog/ItemAccountEiDialog"),
     CtrlApplyInvoiceDialog: () => import("@/components/dialog/CtrlApplyInvoiceDialog"),
    DeleteDialog
  },
  ...
  created() {
    // this.mstData.PK,                 0
    // this.mstData.VAT_RATE,           1
    // this.mstData.REMARK,             2
    // this.mstData.REMARK2,            3
    // this.mstData.TAC_ABACCTCODE_PK,  4
    // this.mstData.AC_CD,              5
    // this.mstData.AC_NM,              6
    // this.txtTRANS_CR_PK,             7
    // this.txtTRANS_CR_CD,             8
    // this.txtTRANS_CR_NM,             9
    // this.mstData.TR_RATE,            10
    // this.mstData.BK_RATE,            11
    // this.mstData.TR_CCY,             12
    // this.mstData.TR_STATUS,          13
    // this.mstData.EI_STATUS,          14
    // this.mstData.INVOICE_FORM_SYMBOL,15
    // this.mstData.TCO_COMPANY_PK,     16
    // this.mstData.CONSIGNOR_PK,       17
    // this.mstData.CONSIGNOR_CD        18
    this.accountPK      = this.paras[4];
    this.accountCD      = this.paras[5];
    this.accountNM      = this.paras[6];
    this.partnerPK      = this.paras[16];
    this.partnerCD      = this.paras[17];
    this.partnerNM      = this.paras[18];
    this.getCcy         = this.paras[12];
  ...
  watch: {
    paras(val) {
      ...
      this.accountPK      = val[4];
      this.accountCD      = val[5];
      this.accountNM      = val[6];
      this.partnerPK      = val[16];
      this.partnerCD      = val[17];
      this.partnerNM      = val[18];
      this.getCcy         = val[12];
  ...
  methods: {
    ...
    onAddAcntMaster() {
        ...
        this.dynamicHeader  = this.BuildDynamicHeader("ITEM");
        this.dialogTitle    = this.$t("control_item");
        this.codeLabel      = this.$t("code");
        this.nameLabel      = this.$t("name");
        this.procedure      = "AC_SEL_6060010_CRCA_CTRLITEM_P";
    ...
    openItemAccDialog(row){
      ...
      /*  this.selectedAccount = { ...row };
        this.selectedRow.data.ITEM_NM == "DEPOSIT ACCOUNT NO") {
    ...
    onDeleteAcntMaster() {
      ...
      this.deletePropsMaster = {
        ...
        para :[ 'd', this.focusedRowKeyMaster, this.txtCrca_Pk, '', '','', '','','' ],
    ...
    callBackDynamicDialog(item) {
      ...
      //const selItem = this.selectedRow
      //const itemAccIdx = this.listCrtItem.findIndex(x => x.PK === selItem.PK)
       //console.log("itemAccIdx : " + itemAccIdx);
      if(this.selectedRow !== undefined)
      {
        ...
        if (idx > -1) {
          ...
          this.listCrtItem.push({
          _rowstatus: "i",
          PK: this._uniqueID(),
          ITEM_NM: item.ITEM_NM,
          ITEM_LNM : item.ITEM_LNM,
          IN_TYPE : item.IN_TYPE,
          DATA_TYPE: item.DATA_TYPE,
          TAC_ABITEM_PK: item.PK,
          TAC_CRCA_PK: this.txtCrca_Pk
         
        });
        }
        if(this.$refs.gridMaster)
        {
          this.$refs.gridMaster.instance.refresh();
        }
      }
      else
      {
    ...
    cellControlDblClick(row) {
     ...
     else if(this.selectedRow.data.ITEM_NM=== 'ITEM')
     {
        this.dynamicHeader  = this.BuildDynamicHeader("PAYMENT METHOD");
        this.dialogTitle    = this.$t("payment_method");
        this.codeLabel      = this.$t("payment_id");
        ...
        this.multiSelectMode = false;
        this.$refs.refDynamicDialog.dialogIsShow = true;
     }else if (this.selectedRow.data.ITEM_NM == 'APPLY INVOICE') {
                this.accountItem = {
                    PK: this.accountPK,
                    CODE: this.accountCD,
                    NAME: this.accountNM
     ...
     returnApplyInvoiceInfo(item) {
            for (let i = 0; i < this.listCrtItem.length; i++) {
                var listCrtItem_ITEM_NM = this.listCrtItem[i].ITEM_NM;
               if (listCrtItem_ITEM_NM === 'APPLY INVOICE') {
                    this.listCrtItem[i].ITEM = item.INVOICE_NO + ' [ ' + item.CCY + ' ][ ' + item.END_TRANS_AMT2 + ' ]';
                } 
                if (this.listCrtItem[i]._rowstatus !== "i") {
                    this.listCrtItem[i]._rowstatus = "u";
                }
    ...
    async getAcntIDList(paras) {
      const dso = {
        type: "grid",
        selpro: "AC_SEL_6060010_CRCA_CTRLITEM",
    ...
    async saveMaster() {
      ...
      if(dataIsModified) {
        const dso = {
          type: 'grid',
          selpro: 'AC_SEL_6060010_CRCA_CTRLITEM',
          updpro: 'AC_UPD_6060010_CRCA_CTRLITEM',
