<template>
  <v-container fluid class="pa-0">
      ...
      <v-col cols="12">
        ...
        <v-row align="center" justify="center">
          <v-col lg="3" cols="12">
            ...
            <v-text-field clearable dense hide-details :label="$t('pl_cd')" @dblclick="openPartnerDialog"
              v-model="txtPL_CD"></v-text-field>
          </v-col>
          ...
          <v-col lg="7" cols="12" class="text-right">

          </v-col>
          <v-col lg="1" cols="12" class="text-right">
            <v-btn icon tile :color="currentTheme" :disabled="false" @click="onSavePL">
              <v-icon>mdi-content-save</v-icon>
            </v-btn>
          </v-col>
          <v-col cols="12" class="py-0">
            <v-card tile v-resize="onResize">
              <DxDataGrid column-resizing-mode="widget" ref="grdPL" :allow-column-resizing="true" :cache-enabled="false"
                :column-auto-width="$vuetify.breakpoint.smAndDown" :data-source="plList" :height="limitHeight" :loadPanel="{
                  enabled: true,
                  text: $t('is_loading', 'common')
                }" :no-data-text="$t('no_data', 'common')" :show-borders="true" :show-column-lines="true"
                :show-row-lines="true" :columns="headerGrid" @row-updated="checkUpdatedItem"
                @selection-changed="selectionChanged" @cellDblClick="openDialog" :onRowPrepared="onRowPrepared"
    ...
    <v-text-field dense hide-details readonly :label="$t('pl_nm')"  v-model="txtPL_NM"
      v-show="false"></v-text-field>
    <employee-dialog ref="empDialog" @callBackData="mappingEmp"></employee-dialog>

    <cost-center-dialog ref="refPLCenter" :companyPK="tco_company_pk"
      @returnData="mappingAccount"></cost-center-dialog>

    <cost-center-dialog ref="refPLCenter1" :companyPK="tco_company_pk"
      @returnData="mappingAccount1"></cost-center-dialog>
  </v-container>
</template>
...
export default {
  ...
  created() {
    ...
    this.gw_Company.value = this.user.TCO_COMPANY_PK;
    this.tco_company_pk = this.paras[16];
    // console.log(this.tco_company_pk  + "  -  " + this.txtCrca_Pk);
    this.getInitCode();
    this.buttonStatus();
    this.onSearchDetails(this.txtCrca_Pk, this.gw_Company.value);
