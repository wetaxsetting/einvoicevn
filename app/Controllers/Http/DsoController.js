...
class DsoController {
  ...
  async htmlToPdf({request, response}) {
    try {
      ...
      const pdf_url =
        APP_URL_LOCAL + '/api/dso/getfiletoken?file_name=' + '/' + folder + '/' + year + '/' + month + '/' + day + '/' + fileName + '&token=' + token;
  ...
  async getFileTokenRealTime({request, response}) {
    try {
      ...
      const file_name = searchParams.get('https://test.einvoicepro.webcashvietnam.com/api/dso/getfiletoken?file_name');
      /*const {token, file_name} = request.get(['token', 'file_name']);
      const token2 = token
        .replace(/p1L2u3S/g, '+')
        .replace(/s1L2a3S4h/g, '/')
