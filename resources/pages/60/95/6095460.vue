          ...
          <v-row dense>
            <v-col md="9" class="pl-3 pr-3 pt-2">
              <BaseSelect
                ...
                :lstData="dataSearchList.companyList"
                v-model="modelSearch.COMPANY_PK" />
            </v-col>
            ...
            <v-col md="6" class="pl-3">
              <BaseSelect
                outlined
                :label="$t('status')"
                :lstData="dataSearchList.statusList"
              ...
              <v-row dense class="pt-1">
                ...
                <v-col md="8" class="pr-3">
                  <GwFlexBox justify="end">
                    <BaseButton
                      icon_type="xml"
                      :btn_text="$t('view_xml_04_ss')"
                      @onclick="onClick('viewXML_04_SS')"
                      :disabled="invoiceStatus == 7 || invoiceStatus == 0" />
              ...
              <v-row dense class="pl-3 pr-3 pt-3">
                <v-col md="2">
                  <BaseSelect
                    outlined
                    :label="$t('version')"
                    :lstData="dataMasterList.versionList"
                    v-model="modelMaster.PBAN"
                    item-text="NAME"
                    item-value="CODE" />
                </v-col>
                <v-col md="2"></v-col>
                <!-- <v-col md="1"></v-col> -->
                <v-col md="4">
                  <BaseInput outlined :label="$t('represent')" v-model="modelMaster.SLLR_RPPR_NM" />
                </v-col>
                <v-col md="4">
                  <BaseInput outlined :label="$t('position')" v-model="modelMaster.SLLR_POSITION" />
                </v-col>
              </v-row>
              <v-row dense class="pl-3 pr-3">
                <v-col md="4">
                  <BaseSelect
                    ...
                    :lstData="dataMasterList.companyList"
                    v-model="modelMaster.TEI_COMPANY_PK"
                    readonly />
                </v-col>
                <v-col md="4">
                  <BaseSelect
                    outlined
                    :label="$t('tax_office')"
                    v-model="modelMaster.MCQT"
                    :lstData="dataMasterList.taxOfficeList"
                    item-text="NAME"
                    item-value="CODE" />
                </v-col>
                <v-col md="4">
                  <BaseSelect
                    outlined
                    :label="$t('form_no')"
                    v-model="modelMaster.MSO"
                    :lstData="dataMasterList.fromNoList"
                    item-text="NAME"
                    item-value="CODE" />
                </v-col>
              </v-row>
              <v-row dense class="pl-3 pr-3">
                ...
                <v-col md="4">
                  <BaseSelect
                    ...
                    v-model="modelMaster.TEN_CD"
                    :lstData="dataMasterList.declarationNameList"
                    item-text="NAME"
                    item-value="CODE" />
                </v-col>
                <v-col md="1"></v-col>
              ...
              <v-row dense class="pl-3 pr-3">
                <v-col md="4">
                  <BaseSelect
                    outlined
                    :label="$t('categories')"
                    v-model="modelMaster.LOAI"
                    :lstData="dataMasterList.categoriesList"
                    item-text="NAME"
                    item-value="CODE" />
                </v-col>
                <v-col md="4">
            ...
            <v-card>
              <v-row class="pt-3" dense>
                <v-col md="7" class="d-flex pl-2">
                  ...
                  <BaseButton
                    icon_type="eye_on"
                    :btn_text="$t('preview_bb_replace')"
                    @onclick="onClick('previewBB_Replace')"
                    :disabled="invoiceStatus == 7 && invoiceType != 3" />
    ...
    <v-dialog v-model="dlg_View" max-width="1000">
      <v-card outlined tile v-resize="onResize">
        ...
        <v-container fluid class="pt-0">
          ...
          <v-row>
            <v-col md="4">
              <BaseSelect :label="$t('form_no')" v-model="form_no_pop" :lstData="list_form_no_pop" item-text="NAME" item-value="VAL" />
            </v-col>
            <v-col md="3">
              <BaseSelect
                :label="$t('serial_no')"
...
export default {
  ...
  methods: {
      ...
      switch (pos) {
        ...
        case 'saveMaster':
          if (this.modelMaster.MCQT == '00') {
            ...
            break;
          } else if (this.modelMaster.TEN_CD == '00') {
          ...
          if (this.selectedTable == 'G') {
            if (!this.modelMaster.PK) {
              ...
              this.$refs.grdDetail.getDataSource().forEach(e => {
                if (!e.TEI_WT_NOTE_SS_M_PK) {
                  e.TEI_WT_NOTE_SS_M_PK = this.modelMaster.PK;
                  e.TEI_WT_NOTE_SS_M_PK = this.modelMaster.TEI_COMPANY_PK;
                }
              });
              this.dlg_View = true;
              let _break = false;
            ...
            this.$refs.grdDetail.addRowStruct({
              ...
              CUSTOMER_NM: '',
              // TTHAI: "",
              TEI_WT_NOTE_SS_M_PK: this.modelMaster.PK,
              BUYER_REPRESENT: '',
              BUYER_POSITION: '',
            });
          }
    ...
    async dsoMaster(action) {
      ...
      let decl_ten = this.dataMasterList.declarationNameList.find(item => item.CODE == this.modelMaster.TEN_CD);
      this.modelMaster.TEN = decl_ten.NAME;
      await this._dsoCall(
        {
          type: 'control',
    ...
    onSelect() {
      ...
      for (let i = 0; i < selectedData.length; i++) {
        this.$refs.grdDetail.onAdd({
          ...
          CUST_NM: selectedData[i].CUST_NM,
          TEI_WT_NOTE_SS_M_PK: this.modelMaster.PK,
          BUYER_POSITION: selectedData[i].BUYER_POSITION,
          BUYER_RPPR_NM: selectedData[i].BUYER_REPRESENT,
        });
      }
      this.dlg_View = false;
    },
    ...
    async initDataList() {
      ...
      const results = await this._getCommonCode2(
        ['ACEI0010', 'ACEI0120', 'ACEI0190', 'ACEI0170', 'ACEI0180', 'ACEIS310', 'ACEI0210', 'ACEI0220'],
    ...
    onGeneralXML() {
      ...
      for (let j = 0; j < this.$refs.grdDetail.getDataSource().length; j++) {
        const objDataDetails = {
          tax_confirmation_code: this.$refs.grdDetail.getDataSource()[j].MCQTCAP,
    ...
    async OnCheckingDec() {
      var tempTradeCode = this.$refs.grdSearch.getDataSource().find(item => item.PK == this.modelMaster.PK);
      ...
      if (this.modelMaster.PK) {
        let res = await this.$axios.$post('/einvoice/checkinformadjustinvoice', {
          ...
          para: {
            ...
            tei_company_pk: this.modelMaster,
            TEI_COMPANY_PK,
    ...
    async onSuccessissueXmlList(data) {
      ...
      const dso_process_check_serialno = {
        ...
        para: [this.modelMaster.TEI_COMPANY_PK, data.serial_number, data.not_before, data.not_after],
      };

      const check_serial_no_result = await this._dsoCall(dso_process_check_serialno, 'select', false);
