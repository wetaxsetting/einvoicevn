...
const EiExcelConverter = use('App/Helpers/EiWTExcel04SS_BBRConverter');
class EiExcelHandler {
  ...
  async getEinvoice(tei_einvoice_m_pk, p_language, p_crt_by, _db2 = 'N') {
    try {
      ...
      let companyName = '';
      let backgroundRow = 0;
      //  tei_einvoice_m_pk = "";  // hóa đơn replace

      const einvoiceMasterData = await DBService.callProcCursor('EI_SEL_WT_04SS_BBR_M', [tei_einvoice_m_pk], p_language, p_crt_by, _db2);
      const einvoiceDetailData = await DBService.callProcCursor('EI_SEL_WT_04SS_BBR_D', [tei_einvoice_m_pk], p_language, p_crt_by, _db2);

      const einvoiceDetailDataReplace = await DBService.callProcCursor('EI_SEL_04SS_BBR_D_WT', [tei_einvoice_m_pk], p_language, p_crt_by, _db2);
      ...
      reportPath = 'report/60/95/ex_04ss/HD_WT_04SS_6.xlsx';
      reportSheet = 'Invoice';
