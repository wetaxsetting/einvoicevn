  ...
  export default {
    ...
    methods: {
        ...
        async getFormList() {
            const dso = {
            type: 'list',
            selpro: 'HR_SEL_8010010_FORM_LIST',
            para: null
            }
            const result = await this._dsoCall(dso, 'select', false)
            if (result) {
      ...
      async createHeader(){
        ...
        this.header1 = [
            { title: ("form_name"), field: "TES_OBJ_PK", editable: true, type: "list", datasource: { KEY: 'PK', VALUE: 'FORM_NAME', data: this.formList }, width: 200 },
            { title: ("use_yn"), field: "USE_YN", editable: true, type: "boolean", width: 80 },
            { title: ("code"), field: "CODE", editable: true, type: "", width: 80 },
      ...
      async onSearch() {
        ...
        const dso = {
          type: 'grid',
          selpro: 'HR_SEL_8010010_REPORT_MANAGE',
          para: pa
        }
      ...
      onDelete(){
        ...
        const dso = {
          type: 'grid',
          selpro: 'HR_SEL_8010010_REPORT_MANAGE',
          updpro: 'HR_UPD_8010010_REPORT_MANAGE',
          para: pa,
          elname:[
            "_rowstatus", 'PK', 'TES_OBJ_PK', 'USE_YN', 'CODE', 'TAB', 'REPORT_NM', 'REPORT_LNM', 'REPORT_FNM', 'PATH_STANDARD', 'PATH_ADVANCE', 'VERSION', 'EXCEL_JSON', 'TCO_COMPANY_PK', "EXTENSION"
          ],
        }
      ...
      async onSave() {
        ...
        const dso = {
          ...
          requirecol:[
            'TES_OBJ_PK', 'REPORT_NM', 'PATH_STANDARD'
          ]
        }
