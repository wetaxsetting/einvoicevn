                ...
                <v-row dense align="center" justify="space-between">
                  ...
                  <v-col lg="5" cols="12">
                    ...
                    <v-btn
                      ...
                      @click="OnSendBusPartner"
                      v-if="btnSend"
                      >{{ $t('interface_einvoice') }}</v-btn
                    >
                    <!-- Update -->
                    <v-btn
                      depressed
                      ...
                      :color="currentTheme"
                      :disabled="isProcessing"
                      @click="OnUpdateBusPartner"
                      v-if="btnUpdate"
                      >{{ $t('update_einvoice') }}</v-btn
                    >
                    <!-- check status Cancel -->
                    <v-btn depressed x-small class="white--text mr-2" :color="currentTheme" :disabled="isProcessing" v-show="false">{{
                      $t('check_status_einvoice')
                    }}</v-btn>
                    <!-- Search -->
                  ...
                  <v-col lg="2" cols="12">
                    <v-select
                      ...
                      v-model="lstTransaction"></v-select>
                  </v-col>
                  <!-- Use Y/N -->
                  <v-col lg="1" cols="12">
                    <v-select
                  ...
                  <v-col lg="1" cols="12" class="py-1">
                    <v-select
                      ...
                      item-value="PK"
                      item-text="TEXT"
                      :label="$t('interface_yn')"
                      :items="interfaceYNList"
                      v-model="lstInterfaceYN"
                      @change="onChangeButton"></v-select>
                  </v-col>
                  <!-- row 3 -->
                  <v-col lg="1" cols="12">
...
export default {
  ...
  methods: {
    ...
    onCellPrepared({column, cellElement, rowType}) {
      if (
        (rowType === 'data' && column.dataField === 'VAT_RATE') ||
        column.dataField === 'SERIAL_NO' ||
        column.dataField === 'ACCD_DR' ||
        column.dataField === 'ACCD_CR1' ||
        column.dataField === 'ACCD_CR2' ||
        column.dataField === 'ITEM_UOM'
      ) {
    ...
    objectToXml(obj) {
      ...
      for (let prop in obj) {
        ...
        xml += '<' + prop + '>';
        if (typeof obj[prop] == 'object') xml += this.objectToXml(new Object(obj[prop]));
        else xml += obj[prop];
        xml += '</' + prop + '>';
      }
      return xml;
    ...
    async insertStatusPartner() {
      ...
      const dso = {
        ...
        para: [this.txtPartnerPk, this.txt_tei_customer_pk, this.txtTei_Company_PK, this.lstCompany, this.selectedRowKeys_Partner.length],
      };
