<template>
  <v-dialog id="account-get-s-p-dialog" max-width="1000" v-model="dialogIsShow">
    <v-card>
...
export default {
  ...
  watch: {
    dialogIsShow(val) {
      if(val === true) {
          //console.log("sel_pro " + this.sel_pro);
          this.dataList= [];
          this.selectedDatas=[]; 
  ...
  methods: {
    async onSearch() {
      //p_code,p_acc_nm,p_acc_unm,p_leaf_yn,p_use_yn,p_compk,p_user_id,p_Year
      var p_user_id = this.user.USER_ID; 
      var p_compk   = this.user.TCO_COMPANY_PK; 
      var  _para = [ this.txtAccountCode,this.txtAccountName,this.txtAccountUpperName,'Y',this.selectedAccountType,p_compk,p_user_id,this.month.replace("-", "")];
      if(this.group_code != 0)
      {
           _para = [ this.txtAccountCode,this.txtAccountName,this.txtAccountUpperName,'Y',this.selectedAccountType,p_compk,p_user_id,this.month.replace("-", "")];
      }
