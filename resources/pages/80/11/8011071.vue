            ...
            <v-col cols="12" lg="4" class="px-1">
                <v-card class="px-0 py-0 100vh " :height="limitHeight">
                    ...
                    <v-row no-gutters class="flex-row" v-if="report1 && report1.length > 0">
                        <v-col cols="11" class="px-1 py-1">
                            <v-sheet class="d-flex">
                                <BaseSelect  :outlined="true" item-value="PK" item-text="TEXT" :label="$t('report')" :lstData="report1" v-model="selectedReport" > </BaseSelect>
                            </v-sheet>
                        </v-col>
                        <v-col cols="1" class="px-1 py-1">
                            <BaseButton btn_type="icon" icon_type="excel" :btn_text="$t('excel')" />
                        </v-col>
                    </v-row>
...
export default {
    ...
    data: () => ({
        ...
        dso: {
            type: 'control',
            selpro: 'HR_SEL_8011071_ORG_INFO_BIZ',
            updpro: 'HR_UPD_8011071_ORG_INFO_BIZ',
        }
    }),
    computed:
    {
    ...
    methods: {
        ...
        async prepareCommonData() {
            ...
            this.report1 = await this._getReportList('8010070', 'ORGANIZATION', this.selectedCompany);
        ...
        async onProcessChange(){
            const dso = {
                type: 'process',
                updpro: 'HR_PRO_8011071_ORG_INFO_V2',
