...
class Utils {
  ...
  async putFileRootPath(file, folder) {
    ...
    const imageExt = type === 'string' ? file.split('?')[0].split('.').pop() : file.extname;
    ...
    let savePath = `${this.ROOT_DIR_FILES}/${folder}/${year}/${month}`; //${this.ROOT_DIR_FILES.replace('/', '')}/${folder}/${year}/${month}`;
  ...
  translate(_key = ' ', _dictionaryList = '', _lang = '') {
    ...
    if (_dictionaryList.length > 0) {
      const found = _dictionaryList.find(
        item =>
          item.ID.toString().toUpperCase().trim() == _key.toString().toUpperCase().trim() &&
          item.LANGUAGE.toString().toUpperCase().trim() == _lang.toString().toUpperCase().trim(),
      );
      if (found) {
        text = found.MESSAGE;
        if (text) return text;
  ...
  async putExcelRootPath(file, folder, type_insert) {
    try {
      const type = typeof file;
      const clientName = type === 'string' ? file.split('?')[0].split('.').pop() : file.clientName;
      let path_result = ``;
      let path = ``;
